---
title: "Secrets"
description: "Storing and using secrets through Kozmo"
---

In various surfaces in Kozmo, you may be asked to input config for certain integrations
...
we created a way to store your secrets in the Kozmo database.

<Warning>
  Your secrets are encrypted before being stored into the Kozmo database. The encryption key
  will be stored in the Kozmo data folder which by default will be created at `~/.kozmo_data`. If
...
![Create Secrets](https://github.com/kozmoai/assets/blob/main/create-secrets.png?raw=True)
...
Secrets are stored to the Kozmo database. If you are using the same database across multiple
Kozmo environments, i.e. development and production, the secrets will not be shared across the environments
unless you shared the same Kozmo data directory for every environment.
...
You can use the following syntax to have Kozmo interpolate the secret when reading from the config:

```yaml
field_name: "{{ kozmo_secret_var('your_secret_name') }}"
```

![Using Secrets](https://github.com/kozmoai/assets/blob/main/using-secrets.png?raw=True)

You can also fetch the secret value in a Kozmo code block by importing a helper method:

```python
from kozmo_ai.data_preparation.shared.secrets import get_secret_value
