...
<Warning>
    **Important**
    ...
    `docker run -d --env-file .env -e SERVER_LOGGING_TEMPLATE=$'%(asctime)s\t\t[%(name)25.25s]\t%(levelname)5s: %(message)s' kozmoai/kozmoai:latest`
</Warning>
...
Then you can run command `kozmo clean-old-logs [project_uuid]` to clean up old logs
