...
description: "Debugging common issues in Kozmo and other frequently asked questions."
---
...
To temporarily fix this, open the `/terminal` page in Kozmo.
Then type the following command press enter to run it:

```bash
rm -rf kozmo_data/[project_name]/pipelines/[pipeline_name]/.variables
```

<Info>Change `[project_name]` to your Kozmo project name and change `[pipeline_name]` to the name of the pipeline that is freezing the browser.</Info>

The above command will remove the data output files for each block in your pipeline.

---

## Can’t connect to local database from within Kozmo via Docker

If you’re running Kozmo via Docker and you’re trying to connect to a local database outside of
...
Thankfully, [Kozmo Community](https://www.kozmo.ai/chat) member [Josh Pachner](https://kozmoai.slack.com/team/U05B9DLK0N7) has a solution.
...
## Kozmo doesn't load properly when running locally

When attempting to connect to Kozmo with a port _other_ than `6789`, the browser may not render the page correctly with `localhost:port`.

Instead, use `127.0.0.1:port` to connect to Kozmo.
...
docker run -it -p 6789:6789
    ...
    -v $(pwd):/home/src kozmoai/kozmoai \
    /app/run_app.sh kozmo start [project_name]
```

or, using `docker compose`, add a volume mount to your Kozmo service. The following is an example of mounting the `~/.aws` directory for authenticating via the AWS CLI:
...
## "Run block" button doesn't work when running Kozmo with Nginx

If Kozmo terminal or "Run block" button doesn't work, it usually means there's a WebSocket connection issue. If Kozmo server is deployed with Nginx, you can add the below config to Nginx config to enable WebScoket connection.
...
If your block gets stuck in running status in the Kozmo editor, you can try the following things:
...
* Check whether the "Compute management" feature is enabled in the project settings. This feature sometimes causes the slowdown of the block execution in the Kozmo editor (notebook).


## Server fails to start or keeps restarting due to healthcheck failures

When deploying your Kozmo server on Cloud (Kubernetes, AWS, GCP, Azure, etc.) with Terraform scripts or Helm chart, there're health checks set up for the
