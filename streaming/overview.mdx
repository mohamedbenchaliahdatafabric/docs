...
### Executor type
When running Kozmo on Kubernetes cluster, you can also configure streaming pipeline to be run on separate k8s pods by setting `executor_type`
...
Follow this [doc](contributing) to add a new source or destination (sink) to Kozmo streaming pipeline.
