...
* `buffer_size_mb` and `buffer_timeout_seconds`: Kozmo puts messages in a buffer before uploading to Google Cloud Storage.
  You can configure the size and timeout of the buffer to control the file size and the delay.
* `path_to_credentials_json_file`: Google service account credential json file. If Kozmo is running on GCP, you can leave this value null and then Kozmo will use the instance service account to authenticate.
