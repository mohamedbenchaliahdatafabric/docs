...
* `buffer_size_mb` and `buffer_timeout_seconds`: Kozmo puts messages in a buffer before uploading to S3.
...
1. Add the following keys and values to your environment variables
  ...
  * `AWS_REGION`
2. If you deploy Kozmo on AWS ECS cluster, you can use ECS execution task role to authenticate.
