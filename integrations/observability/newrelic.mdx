...
An example `example_newrelic.ini` file is attacehd in Kozmo repo under folder `integrations/newrelic`.

Add path of your newrelic configuration into environment variable `NEW_RELIC_CONFIG_PATH`. 

## Using New Relic in Kozmo

Monitoring with New Relic will be enabled the next time you start the Kozmo server.

The following logs and errors will be reported to New Relic:

* Trigger runs.
* Kozmo scheduler.
* `kozmo run` CLI command.
