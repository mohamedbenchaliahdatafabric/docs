...
## Using Sentry in Kozmo

Once you set the `SENTRY_DSN` environment variable, the Sentry integration will be
enabled the next time you start the Kozmo server.

The following errors will be reported to Sentry:

* Exceptions from trigger runs.
* Exceptions from the Kozmo scheduler.
* Exceptions from runs started by the `kozmo run` CLI command.