---
title: "Run Kozmo pipelines in Prefect"
sidebarTitle: "Prefect"
description: "We support running the pipeline in Prefect flows."
---

You need to firstly install `kozmo_ai` library by adding `kozmo_ai` to your
requirements.txt file. Then you need to download the kozmo pipeline code into
your Prefect directory. You can achieve it by using a git submodule in your
Prefect directory.

We provide two ways to run kozmo pipelines in Airflow.
...
with Flow('My Kozmo Flow') as flow:
    run_pipeline()
...
kozmo_task = ShellTask(return_all=True)
...
with Flow('My Kozmo Flow') as flow:
    run_pipeline = kozmo_task(command=f'kozmo run {project_path} {pipeline_name}')
