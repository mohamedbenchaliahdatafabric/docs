...
sidebarTitle: "Spark"
description:
  "This is a guide for using Spark (PySpark) with Kozmo in different cloud
...
When you run Kozmo and Spark in the same Kubernetes cluster, you can set the environment variable `SPARK_MASTER_HOST`
to the url of the master node of the Spark cluster in Kozmo container. Then you'll be able to connect Kozmo to your Spark
cluster and execute PySpark code in Kozmo.

Here is an overview of the steps required to use Kozmo with Spark in Kubernetes cluster.
...
### 2. Build Kozmo docker image with Spark environment
* Download the [Dockerfile template](https://github.com/kozmoai/kozmoai/blob/master/integrations/spark/Dockerfile) for Kozmo with Spark environment.
* Change the current directory to the directory that contains the `Dockerfile`. Build the docker image with the command `docker build -t kozmo_spark .`

### 3. Run Kozmo in Kubernetes cluster with the `SPARK_MASTER_HOST` environment variable
...
* Then deploy Kozmo to Kubernetes with the updated yaml file.
...
spec:
  ...
  - name: kozmo-fs
    hostPath:
      path: /path/to/kozmo_project
  serviceAccountName: kozmo-user
```

Updates
* In Kozmo version `0.8.83` and above, you don't need to specify the environment variable anymore.
...
### 4. Run PySpark code in Kozmo
When Kozmo pod is running, you can create a "Standard (batch)" pipeline (`python` kernel) and then write PySpark code in any blocks.
...
### 1. Build Kozmo docker image with Spark environment
* Download the [Dockerfile template](https://github.com/kozmoai/kozmoai/blob/master/integrations/spark/Dockerfile) for Kozmo with Spark environment.
* Change the current directory to the directory that contains the `Dockerfile`. Build the docker image with the command `docker build -t kozmo_spark .`

### 2. Start Kozmo with `SPARK_MASTER_HOST` environment variable
Type this command in your terminal to start Kozmo using docker

```bash
docker run -it --name kozmo_spark -e SPARK_MASTER_HOST='local' -p 6789:6789 -v $(pwd):/home/src kozmo_spark \
...
### 3. Run PySpark code in Kozmo
When Kozmo is running, you can create a "Standard (batch)" pipeline (`python` kernel) and then write PySpark code in any blocks.
...
### 1. Build Kozmo docker image with Spark environment
* Download the [Dockerfile template](https://github.com/kozmoai/kozmoai/blob/master/integrations/spark/Dockerfile) for Kozmo with Spark environment.
* Build the docker image with the command `docker build -t kozmo_spark .`

### 2. Start Kozmo using docker with the following command in your terminal

```bash
docker run -it --name kozmo_spark -p 6789:6789 -v $(pwd):/home/src kozmo_spark \
  /app/run_app.sh kozmo start demo_project
```
Notes
* `demo_project` is the name of your project, you can change it to anything you want
...
### 4. Run PySpark code in Kozmo
When Kozmo is running, you can create a "Standard (batch)" pipeline (`python` kernel), and add a block,
...
```python
from kozmo_ai.data_preparation.repo_manager import RepoConfig, get_repo_path
from kozmo_ai.services.spark.config import SparkConfig
from kozmo_ai.services.spark.spark import get_spark_session
...
### 1. Build Kozmo docker image with a Hadoop Yarn environment
* Download the [Dockerfile template](https://github.com/kozmoai/kozmoai/blob/master/integrations/hadoop/Dockerfile) for Kozmo with Hadoop environment.
* Build the docker image with the command `docker build -t kozmo_hadoop .`


### 2. Start Kozmo with the following command in your terminal using docker

```bash
docker run -it --name kozmo_hadoop -p 6789:6789 -v $(pwd):/home/src kozmo_hadoop \
  /app/run_app_with_hadoop.sh kozmo start demo_project
```
Notes
* `demo_project` is the name of your project, you can change it to anything you want

### 3. Run PySpark code in Kozmo using Hadoop Yarn
...
Here is an overview of the steps required to use Kozmo locally with Spark in AWS:

1. [Create an S3 bucket for Spark](#1-create-an-s3-bucket-for-spark)
2. [Start Kozmo](#2-start-kozmo)
...
Using Kozmo with Spark is much easier if you use Docker.
...
Once you’ve acquired those credentials, you can switch your kernel in your Kozmo notebook
to `pyspark` kernel. Kozmo will automatically creates the EMR cluster when you switch to `pyspark`
...
This will allow your locally running Kozmo to remotely access AWS EMR.

### 6. Sample pipeline with PySpark code

If you aren’t using Docker and you installed Kozmo using `pip`, you must run the
...
Make sure the EMR cluster is accessible from the Kozmo server. You can verify whether Kozmo server's
