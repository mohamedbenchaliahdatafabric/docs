...
<Note>Event triggered pipelines only work when Kozmo is running in a cloud provider like AWS, GCP, or Azure.</Note>

## TLDR

1. Create a CloudWatch event rule.
1. Add AWS credentials to environment variables for Kozmo to use.
...
![](https://github.com/kozmoai/assets/blob/main/guides/triggers/events/create-event.png?raw=true)

## Add AWS credentials to environment variables

If you’re using Kozmo in Docker, add the following environment variables when you run
Kozmo:
...
If you’re not using Docker to run Kozmo, run the following commands:
...
For event trigger, Kozmo uses a Lambda Function to relay the event from CloudWatch to Kozmo.

If you deploy Kozmo to AWS with [terraform scripts](https://github.com/kozmoai/kozmoai-terraform-templates/tree/master/aws),
you can uncomment the code in the following places and run `terraform apply` to create the Lambda Function:
* https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/lambda.tf#L1-L23
* https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/iam.tf#L29-L73
* https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/main.tf#L46-L47
* Add the `LAMBDA_FUNCTION_ARN` and `LAMBDA_FUNCTION_NAME` environment variables to https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/env_vars.json
  ```json
  {
...
If you don't use the terraform script to deploy Kozmo, you can set up the Lambda Function with the following steps:
1. Create a Lambda Function.
    1. Select `Python 3.9` as Runtime.
    1. Use the same VPC and subnets of your Kozmo service.
    1. Create a security group. Edit your security group of the Kozmo instance to allow traffic from the Lambda Function's security group.
1. Set the environment variable `KOZMO_API_HOST` in the Lambda Function.
1. Copy the code from the [event_handler.py](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/python/event_handler.py) to
   your Lamdba Function.
1. Test the connection by the executing Lambda Function with a test event json.
1. Add environment variables `LAMBDA_FUNCTION_ARN` and `LAMBDA_FUNCTION_NAME` to your Kozmo service.
...
![](https://github.com/kozmoai/assets/blob/main/guides/triggers/events/create-trigger.png?raw=true)
