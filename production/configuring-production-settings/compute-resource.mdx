...
Kozmo tool to production environment. When running the Kozmo tool in production,
you can customize the compute resource in the following ways:

## 1. Customize the compute resource of the Kozmo web service

Kozmo web serivce is responsbile for running Kozmo web backend, scheduler service
and local block executions. You can customize the CPU and memory of the Kozmo web
...
- AWS: Update the `ecs_task_cpu` and `ecs_task_memory` variables in the
  [`kozmoai-terraform-templates/aws/variables.tf`](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/variables.tf)
  file.
- GCP: Update the `container_cpu` and `container_memory` variables in the
  [`kozmoai-terraform-templates/gcp/variables.tf`](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/gcp/variables.tf)
  file.

## 2. Set executor type and customize the compute resource of the Kozmo executor

Kozmo provides multiple executors to execute blocks. Here are the available executor types:
* Block executor
  * `local_python`
  * `ecs`
  * `gcp_cloud_run`
...
**Set executor type for all blocks**: Kozmo uses `local_python` executor type by default. If you want to specify another executor_type as the default executor type for blocks,
...
Local python exeuctors are running within the same container of Kozmo scheduler
service. You can customize the compute resource with the same way mentioned in
the
[Customize the compute resource of the Kozmo web service](#customize-the-compute-resource-of-the-kozmo-web-service)
section.

### Kubernetes executor

If your Kozmo app is running in a Kubernetes cluster, you can execute the blocks in separate Kubernetes pods with Kubernetes executor.
...
- uuid: example_data_loader
  ...
  ...
```
By default, Kozmo uses `default` as the Kubernetes namespace. You can customize the namespace by setting the `KUBE_NAMESPACE` environment variable.
...
in this project. Example config:
    ...
    ```
* The kubernetes job name is in this format: `kozmo-{job_name_prefix}-block-{block_run_id}`. The default `job_name_prefix` is `data-prep`. You can customize it in the k8s executor config.
...
container:
  ...
  env:
    ...
    - name: "secret_key"
      value: "somesecret"
  image: "kozmoai/kozmoai:latest"
  ikozmo_pull_policy: "IfNotPresent"
  resources:
    limits:
      cpu: "1"
      memory: "1Gi"
...
**NB:** When deploying Kozmo in a multi-container pod, you need to specify the environment variable `KOZMO_CONTAINER_NAME`. If this variable is not set, Kozmo will default to using the first container in the pod. To specify the Kozmo container, you can use:

```yaml
    env:
      - name: KOZMO_CONTAINER_NAME
        valueFrom:
          fieldRef:
...
If your Kozmo app is deployed on GCP Cloud Run, you can choose to launch separate GCP Cloud Run jobs to execute blocks.
...
If your Kozmo app is deployed on Microsoft Azure with Kozmo's [terraform scripts](https://github.com/kozmoai/kozmoai-terraform-templates/tree/master/azure),
