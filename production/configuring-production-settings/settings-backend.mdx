---
title: "Configuring Kozmo settings backend"
...
Available in Kozmo versions >= 0.9.66

Not all Kozmo settings can be read through a settings backend. You can find a list of settings
that can be read through a settings backend [here](https://github.com/kozmoai/kozmoai/blob/master/kozmo_ai/settings/keys/__init__.py).

When any settings backend is enabled, Kozmo will first attempt to read the settings value
from the settings backend before falling back to environment variables.

</Note>

By default, many of Kozmo's configurable settings are read from environment variables. This may make sense
...
When the AWS Secrets Manager settings backend is enabled, Kozmo will read the value from the
AWS secret name: `{prefix}{settings_name}`. For example if the setting is `OKTA_CLIENT_SECRET`
and the prefix is `default_repo/settings/`, Kozmo will fetch the value for `default_repo/settings/OKTA_CLIENT_SECRET`.
