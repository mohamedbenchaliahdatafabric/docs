...
description: "Create users, manage users, and require sign in to authenticate and use Kozmo."
---
...
If you’re using Kozmo version 0.8.4 or greater, than you can optionally turn on user authentication.
...
You can set the `KOZMO_ACCESS_TOKEN_EXPIRY_TIME` environment variable to customize the expiration time
of a Kozmo access token. The value should be the length of time in seconds.

Defaults to 2592000 which is 30 days.

|   |   |
| --- | --- | 
| Name | `KOZMO_ACCESS_TOKEN_EXPIRY_TIME` |
| Value | `2592000` |

### Running Kozmo in Docker

If you’re running Kozmo using Docker, you can run Kozmo and set the environment variable
...
docker run -it -p 6789:6789 \
  ...
  kozmoai/kozmoai /app/run_app.sh kozmo start demo_project
```

### Running Kozmo without Docker

If you installed Kozmo using `pip`, `conda`, `poetry`, etc. and are running it using the `kozmo start`
...
When Kozmo starts, if there is no existing owner user, then a new user will be created with owner
...
Before using Kozmo, users will have to sign in at `/sign-in`. If you go to a page that requires
authentication and you’re not signed in, you’ll be redirected to the sign in page.

![Sign in page](https://raw.githubusercontent.com/kozmoai/assets/main/users/sign-in.jpg)
...
![Sign in page](https://raw.githubusercontent.com/kozmoai/assets/main/users/users-list.jpg)
...
| `LDAP_ROLES_MAPPING` | Mapping of user LDAP group to Kozmo roles. Must be in JSON string format. | `'{"Admin": ["Admin"]}'` |
...
When LDAP authentication is enabled, users will need to provide their LDAP credentials to log in to the system. Once authenticated, Kozmo will use the authorization filter to determine the user's permissions based on their LDAP group membership.
