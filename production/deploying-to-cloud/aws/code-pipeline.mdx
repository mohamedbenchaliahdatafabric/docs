...
* If you are not using ECS to run Kozmo, then this document will not apply for you.
  You can submit a feature request in our [Slack channel](https://www.kozmo.ai/chat) if you want Kozmo to
...
will need to have a CodeCommit repo already set up that has your Kozmo repository code.

Your CodeCommit repository should have a Dockerfile in the top level of your CodeCommit.
[Here is a reference Dockerfile](https://github.com/kozmoai/docker/blob/master/Dockerfile)
...
phases:
  ...
  build:
    commands:
      ...
      - docker build -t $IKOZMO_REPO_NAME:$IKOZMO_TAG .
      - docker tag $IKOZMO_REPO_NAME:$IKOZMO_TAG $AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IKOZMO_REPO_NAME:$IKOZMO_TAG
  post_build:
    commands:
      - echo Build completed on `date`
      - echo Pushing the Docker image...
      - docker push $AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IKOZMO_REPO_NAME:$IKOZMO_TAG
```
...
Use Terraform scripts in [aws-code-pipeline](https://github.com/kozmoai/kozmoai-terraform-templates/tree/master/aws-code-pipeline)
...
In the file
[./aws/variables.tf](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws-code-pipeline/variables.tf),
...
After running the Terraform script, you should see two AWS CodePipeline pipelines created, `kozmo-codepipeline`
and `kozmo-codepipeline-ecr` (unless you changed the `code_pipeline_name` variable). The `kozmo-codepipeline` pipeline
will be triggered every time a change is detected in the AWS CodeCommit repo. The `kozmo-codepipeline-ecr` pipeline
will be triggered every time a new image is pushed with into the AWS ECR repo. 

![kozmo-codepipeline](https://github.com/kozmoai/assets/blob/main/kozmo-codepipeline.png?raw=True)

![kozmo-codepipeline-ecr](https://github.com/kozmoai/assets/blob/main/kozmo-codepipeline-ecr.png?raw=True)
