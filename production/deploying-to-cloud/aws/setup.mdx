...
Use Terraform scripts in [aws](https://github.com/kozmoai/kozmoai-terraform-templates/tree/master/aws)
...
In the file
[./aws/variables.tf](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/variables.tf),
...
variable "docker_image" {
  ...
  default     = "kozmoai/kozmoai:latest"
}
```
...
1. `docker build --platform linux/amd64 -tag [ikozmo_name]:latest .`
2. `docker tag [ikozmo_name]:latest [registry_uuid].dkr.ecr.[region].amazonaws.com/[ikozmo_name]:latest`
3. `docker push [registry_uuid].dkr.ecr.[region].amazonaws.com/[ikozmo_name]:latest`
...
In the
[`./aws/env_vars.json`](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws/env_vars.json)
...
Once it’s finished deploying, you can access Kozmo in your browser.

1. Open your EC2 dashboard.
2. View all load balancers.
3. Click on the load balancer with the name `kozmo-data-prep` in it (if you
...
From the root directory of Kozmo, run the following commands:

1. Initialize Terraform:

```bash
docker run -i -t -v $(pwd):/kozmo --workdir="/absolute_path_to/kozmoai-terraform-templates/aws" \
...
### Updating Kozmo versions

After Kozmo is running in your cloud, you can update the Kozmo Docker image
...
To enable other IP addresses access to Kozmo, open the security group named
`kozmo-data-prep-sg` to whitelist IPs.
...
7. In "Default actions", select "Forward" and forward to the target group that
   contains your app name (`kozmo-data-prep-production-tg` by default)
...
Kozmo creates an ECS task execution role with the format `{app_name}-execution-task-role`.
...
Use Terraform scripts in [aws-eks](https://github.com/kozmoai/kozmoai-terraform-templates/tree/master/aws-eks)
...
Similar with deploying to ECS cluster, you need to set up the [environment variables](/production/deploying-to-cloud/aws/setup#1-environment-variables) first, and you can customize the Terraform settings in [./aws-eks/variables.tf](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws-eks/variables.tf) file.


Then, follow the steps below to set up EKS cluster and deploy Kozmo.
...
Update the `EFS_VOLUME_ID` in [kube/efs-pvc.yaml](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws-eks/kube/efs-pvc.yaml) to the efs id in terraform output
</Step>
...
Run `kubectl apply -f kube/` to deploy app or use Helm to deploy Kozmo.
</Step>

<Step title="Whitelist IP in EKS node's security group" />

<Step title="Access Kozmo on EKS">
Get the node IP via command `kubectl get nodes -o wide` and access Kozmo with `http://[ip]:{node_port}`
</Step>
