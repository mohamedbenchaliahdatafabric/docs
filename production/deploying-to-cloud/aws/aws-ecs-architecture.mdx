...
The most common AWS deployment of Kozmo is with AWS ECS. Our Terraform scripts manage the process of creating an ECS cluster, a load balancer, and a database. [Here's a link](/production/deploying-to-cloud/aws/resources) to the default output of `terraform plan` on AWS ECS.

<Frame height="400" caption="An architecture diagram of a Kozmo AWS ECS deployment.">
  <img alt="Contributing" src="https://github.com/kozmoai/assets/blob/main/architecture/aws-ecs.png?raw=True" />
</Frame>
...
The Kozmo application runs as multiple containers in an Amazon ECS service.
...
We use EFS to store a persistent volume where the `kozmo_data` for your project is stored. You can learn more about what is stored in `kozmo_data` [here](/design/abstractions/project-structure#project-data). Without EFS, your project data would be lost when your ECS service is terminated. EFS allows us to persist data between containers within ECS.
...
We use a Postgres RDS to hold the the backend database for your Kozmo instance. This database contains the metadata that powers Kozmo. Read more [here](/design/abstractions/project-structure#project-data).
...
The Kozmo application, ALB, ECS, EFS, and RDS are all deployed within a VPC in our Terraform scripts.
