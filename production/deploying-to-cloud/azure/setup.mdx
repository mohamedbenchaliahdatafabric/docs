...
Here is a high level diagram of how Kozmo interacts with various Azure resources deployed using
this Terraform template.

![Kozmo Azure architecture](https://github.com/kozmoai/assets/blob/main/production/deployment/azure/architecture.png?raw=true)

## Pre-requisites

- Download Kozmo maintained [Terraform templates](/production/deploying-to-cloud/using-terraform).
...
[./azure/variables.tf](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/azure/variables.tf)
file.
...
In the file
[./azure/main.tf](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/azure/main.tf),
...
1. Deploy
    ...
    id = "/subscriptions/d17fcd12-b89c-4f81-a221-40b8e768a0e8/resourceGroups/kozmo-data-prep2-production/providers/Microsoft.ContainerInstance/containerGroups/kozmo-data-prep-production"
    ip = "20.3.85.62"
    ```
...
```bash
az container restart --name kozmo-data-prep-production --resource-group kozmo-data-prep-production
```
...
In order to access Kozmo on Azure from another local workstation, you must add
that IP address to the security group named
`kozmo-data-prep-production-nsg-public`.
