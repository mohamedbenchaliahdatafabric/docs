...
Deploy Kozmo to Kubernetes cluster with Helm.
...
# Deploy Kozmo using Helm

Kozmo helm repo: https://kozmoai.github.io/helm-charts/
...
```
helm install my-kozmoai kozmoai/kozmoai
```
...
- Get the `values.yaml` with the command
    ...
    ```bash
    helm show values kozmoai/kozmoai > values.yaml
    ```
    
- Customize the mount volume
  - **Local**: Edit the `volumes` config in `values.yaml` to mount to your Kozmo project path
  - **AWS EKS**: Provision the Elastic File System and create the [PersistentVolumeClaim](https://github.com/kozmoai/kozmoai-terraform-templates/blob/master/aws-eks/kube/efs-pvc.yaml) (using ReadWriteMany access mode). Then edit the `volumes` config to
    ```
    volumes:
      - name: kozmo-fs
...
- Install the Helm charts with updated `values.yaml`
    ```bash
    helm install --values path/to/values.yaml my-kozmoai kozmoai/kozmoai
    ```
    
## Releases

https://github.com/kozmoai/helm-charts/releases
