...
Kozmo supports GitHub Actions for CI/CD workflows. We have templates for deploying to [AWS ECS, GCP Cloud Run, and Azure](https://github.com/kozmoai/kozmoai/tree/master/templates/github_actions).

Before you configure your action, follow the [Kozmo project setup instructions](/production/ci-cd/local-cloud/repository-setup).
...
13. Copy the contents from the GitHub Action YAML file for AWS at
    [templates/github_actions/build_and_deploy_to_aws_ecs.yml](https://github.com/kozmoai/kozmoai/blob/master/templates/github_actions/build_and_deploy_to_aws_ecs.yml),
...
| `CONTAINER_NAME`      | Set this to the name of the container in the containerDefinitions section of your task definition.                                                                                                                                                                                 | `kozmo-data-production-container`     |
| `ECR_REPOSITORY`      | The name of the AWS ECR repository you created to store your Docker images.                                                                                                                                                                                                        | `kozmo-data`                          |
| `ECS_CLUSTER`         | The name of your AWS ECS cluster.                                                                                                                                                                                                                                                  | `kozmo-production-cluster`            |
| `ECS_SERVICE`         | The name of your AWS ECS service.                                                                                                                                                                                                                                                  | `kozmo-production-ecs-service`        |
| `ECS_TASK_DEFINITION` | Go to your AWS ECS task definition for the above service. Click on the **JSON** tab on the task definition detail page. Copy the JSON string content and save it to a file in your root folder containing your Kozmo project. Use the path to that file as the value in this field. | `some_path/ecs-task-definition.json` |
...
3. In the field labeled **Secret**, enter the JSON string containing your GCP
   ...
   `json { "type": "service_account", "project_id": "kozmo-123456", "private_key_id": "...", "private_key": "-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n", "client_email": "kozmo@kozmo-123456.iam.gserviceaccount.com", "client_id": "...", "auth_uri": "https://accounts.google.com/o/oauth2/auth", "token_uri": "https://oauth2.googleapis.com/token", "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs", "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/kozmo%40mage-123456.iam.gserviceaccount.com" } `
...
8. Copy the contents from the GitHub Action YAML file for GCP at
   [templates/github_actions/build_and_deploy_to_gcp_cloud_run.yml](https://github.com/kozmoai/kozmoai/blob/master/templates/github_actions/build_and_deploy_to_gcp_cloud_run.yml),
   and paste it into the textarea.
9. Change the following values under the key labeled `env`:
   ...
   | Key                             | Description                                                                                 | Sample value  |
   | ------------------------------- | ------------------------------------------------------------------------------------------- | ------------- |
   | `GAR_LOCATION`                  | Region that Kozmo is already deployed in.                                                    | `us-east1`    |
   | `GOOGLE_CLOUD_RUN_SERVICE_NAME` | The name of your Google Cloud Run service. Go to the Google Cloud Run dashboard to find it. | `kozmo-data`   |
   | `IKOZMO`                         | The name of the Docker image you pushed to the above GCP Artifact Registry.                 | `kozmoai`      |
   | `PROJECT_ID`                    | Project ID of where you launched Kozmo using Terraform.                                      | `kozmo-123456` |
   | `REPOSITORY`                    | The name of your GCP Artifact Registry that is storing your Docker image.                   | `kozmoprod`    |
...
Kozmo can be deployed to Azure using GitHub Actions.

Our template for deploying to an Azure Container instance is located [here](https://github.com/kozmoai/kozmoai/blob/master/templates/github_actions/build_and_deploy_to_azure_container_instance.yml) and based off the "GitHub Workflow" option in Azure's docs.
