...
<video
    ...
    src="https://github.com/kozmoai/assets/raw/main/version-control/demo.mp4">
</video>

## Setup

<Frame>
    <img alt="Setup" src="https://github.com/kozmoai/assets/blob/main/version-control/setup.png?raw=true" />
</Frame>
...
authenticate with multiple git providers if you need to. Kozmo will attempt to use the oauth provider
based on the URL of the remote.

<Warning>

Kozmo authenticates with a default GitHub OAuth application that may not be able to access
...
steps in the `Authenticate with GitHub or GHE` section below to connect Kozmo with your OAuth
application.

</Warning>

When creating the OAuth app for GHE, BitBucket, or GitLab, you will need to set the authorized
callback URL to `http(s)://<your-kozmo-url>/oauth`.
...
<Step title="Register Kozmo in GitHub">

In order for Kozmo to authenticate with your GitHub host, you will need to add Kozmo as an
...
You will need to set the following environment variables in your Kozmo environment:
...
<Step title="Add Kozmo as an OAuth consumer">

In order for Kozmo to authenticate with your BitBucket, you will need to add Kozmo as an
...
<Step title="Authenticate Kozmo with Bitbucket">
...
Kozmo will request `read_user`, `write_repository`, and `api` scopes from GitLab in order
...
<Step title="Add Kozmo Oauth application">

In order for Kozmo to authenticate with your GitLab, you will need to add a Kozmo oauth application.
...
<Step title="Authenticate Kozmo with GitLab">
...
<Frame>
    <img alt="Authenticate" src="https://github.com/kozmoai/assets/blob/main/version-control/authentication.png?raw=true" />
</Frame>

After authorizing Kozmo, you’ll be redirected back to the initial page and an access token will
...
For example, the remote URL for Kozmo is `https://github.com/kozmoai/kozmoai.git`.
...
<Frame>
    <img alt="Branches" src="https://github.com/kozmoai/assets/blob/main/version-control/branches.png?raw=true" />
</Frame>
...
<Frame>
    <img alt="Files" src="https://github.com/kozmoai/assets/blob/main/version-control/files.png?raw=true" />
</Frame>
...
<Frame>
    <img alt="Push" src="https://github.com/kozmoai/assets/blob/main/version-control/push.png?raw=true" />
</Frame>
