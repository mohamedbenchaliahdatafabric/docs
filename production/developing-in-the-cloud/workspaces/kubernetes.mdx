...
<Note>
We recommend creating your ingress separately from the Kozmo Helm chart if you are planning
...
or use [the kozmoai-ingress Helm chart](https://github.com/kozmoai/helm-charts/tree/master/charts/ingress).
</Note>

We recommend using Helm to set up your Kubernetes cluster. You can find our Kozmo Helm
...
env:
  ...
  - name: PROJECT_TYPE
    value: main
  - name: KOZMO_DATABASE_CONNECTION_URL
...
using the kozmoai-ingress Helm chart, you can add the following configuration to your `values.yaml` file:
...
<Frame>
    <img
        ...
        src="https://github.com/kozmoai/assets/blob/main/production/workspaces/workspaces-k8s-settings.png?raw=true"
		/>
</Frame>
...
Kozmo provides some support for managing the lifecycle of the workspace. For Kubernetes workspaces, Kozmo
...
<Note>
When the workspace is stopped, Kozmo will scale down the stateful set to 0, but all the other resources will still exist. The
...
<Frame>
    <img
        ...
        src="https://github.com/kozmoai/assets/blob/main/production/workspaces/lifecycle-auto-termination.png?raw=true"
		/>
</Frame>

Kozmo will automatically stop the workspace after the specified amount of time (max idle time).
...
<Frame>
    <img
        alt="Auto termination configuration"
        src="https://github.com/kozmoai/assets/blob/main/production/workspaces/lifecycle-pre-start.png?raw=true"
		/>
</Frame>
...
def get_custom_configs(kozmo_container_config: Dict) -> Dict:
...
<Frame>
    <img
        alt="Auto termination configuration"
        src="https://github.com/kozmoai/assets/blob/main/production/workspaces/lifecycle-post-start.png?raw=true"
		/>
</Frame>
...
Kozmo provides two configuration fields:
...
2. `Path to hook`: Kozmo will mount the file specified by this setting to the container. You can reference this fild in your
...
Path to hook: /home/src/my_kozmo_project/example_post_start.sh
```
...
When an ingress is provided, the `KOZMO_BASE_PATH` environment variable will be set automatically
