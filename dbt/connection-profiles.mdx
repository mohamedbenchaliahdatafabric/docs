...
Kozmo will look for a file named `profiles.yml` located in your dbt project root
folder.

For example, if your Kozmo project is named `demo_project` and your dbt project
...
Using Kozmo, you may also [interpolate variables](/dbt/variable-interpolation) in your connection profiles.
...
target that Kozmo uses when running a dbt model block.

The command Kozmo executes is `dbt run` with the flag `--target` and the value of
