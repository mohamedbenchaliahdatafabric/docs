...
Kozmo provides data loading clients that simplify loading and exporting data in your pipelines, allowing you to spend more time analyzing and transforming your data for ML tasks. Currently, Kozmo includes clients for the following data sources:
...
Kozmo's data loading clients fall into two categories:
...
While traditional Pandas IO procedures can be utilized to load files into your pipeline, Kozmo provides the `kozmo_ai.io.file.FileIO` client as a convenience wrapper.
...
Loading data from a Snowflake data warehouse is made easy using the `kozmo_ai.io.snowflake.Snowflake` data loading client. In order to authenticate access to a Snowflake warehouse, the client requires the associated Snowflake credentials:

-   Account Username
-   Account Password
-   Account ID (including your region) (Ex: _example.us-west-2_)

These parameters can be manually specified as input to the data loading client (see [Snowflake](#snowflake) API). However we recommend using a [Configuration Loader](#configuration-settings) to handle loading these secrets. If you used `kozmo init` to create your project repository, you can store these values in your `io_config.yaml` file and use `kozmo_ai.io.config.ConfigFileLoader` to construct the data loader client.
...
`kozmo_ai.io.redshift.Redshift`

Handles data transfer between a Redshift cluster and the Kozmo app. Kozmo uses temporary credentials to authenticate access to a Redshift cluster. There are two ways to specify these credentials:
...
Handles data transfer between an S3 bucket and the Kozmo app. The `S3` client supports importing and exporting with the following file formats:
...
If an IAM profile is not set up using `aws configure`, then AWS credentials for accesses the S3 bucket can be manually specified through either Kozmo's [configuration loader](#configuration-settings) system or through keyword arguments in the constructor (see constructor).
...
-   **Args**
    ...
    -   If IAM profile is not set up using `aws configure` and Kozmo's configuration loader is not used, then specify your AWS credentials through the following keyword arguments:
...
`kozmo_ai.io.file.FileIO`

Handles data transfer between the filesystem and the Kozmo app. The `FileIO` client currently supports importing and exporting with the following file formats:
...
`kozmo_ai.io.google_cloud_storage.GoogleCloudStorage`

Handles data transfer between a Google Cloud Storage bucket and the Kozmo app. Supports loading files of any of the following types:
...
`kozmo_ai.io.azure_blob_storage.AzureBlobStorage`

Handles data transfer between an Azure Blob Storage conatiner and Kozmo. Supports loading files of the following types:
-   CSV
-   JSON
-   Parquet
-   HDF5
...
`kozmo_ai.io.google_sheets.GoogleSheets`

Handles data transfer between a Google Sheets spreadsheet and the Kozmo app.
...
The Google Sheets class is defined [here](https://github.com/kozmoai/kozmoai/blob/master/kozmo_ai/io/google_sheets.py).

## BigQuery

`kozmo_ai.io.bigquery.BigQuery`

Handles data transfer between a BigQuery data warehouse and the Kozmo app.
...
`kozmo_ai.io.postgres.Postgres`

Handles data transfer between a PostgreSQL database and the Kozmo app. The `Postgres` client utilizes the following keys to connect the PostgreSQL database.
...
`kozmo_ai.io.mysql.MySQL`

Handles data transfer between a MySQL database and the Kozmo app. The `MySQL` client utilizes the following keys to connect the MySQL database.
...
Handles data transfer between DuckDB database and the Kozmo app.
...
`kozmo_ai.io.snowflake.Snowflake`

Handles data transfer between a Snowflake data warehouse and the Kozmo app. The `Snowflake` client utilizes the following keys to authenticate access and connect to Snowflake servers.
...
Instead, Kozmo provides **configuration loaders** which allow data loading clients to use your secrets without explicitly writing them in code.

<Warning>
  Currently Kozmo only supports AWS Secrets Manager as a configuration loader. If you need to use secrets from Azure Key Vault
  or GCP Secret Manager, you can reference the following docs:
    - [Azure Key Vault](https://docs.kozmo.ai/production/deploying-to-cloud/secrets/Azure)
    - [GCP Secret Manager](https://docs.kozmo.ai/production/deploying-to-cloud/secrets/GCP)
</Warning>
...
```python
from kozmo_ai.io.config import AWSSecretLoader
...
The following are the set of allowed key names that you must name your secrets with in order for Kozmo's configuration loaders to recognize your secrets. In code you can refer to these keys by their string name or using the `kozmo_ai.io.config.ConfigKey` enum. Not all keys need be specified at once - only use the keys related to the services you utilize.
...
```python
from kozmo_ai.io.config import ConfigKey, ConfigFileLoader
...
```python
from kozmo_ai.io.config import ConfigKey, EnvironmentVariableLoader
...
```python
from kozmo_ai.io.config import ConfigKey, AWSSecretLoader
