...
icon: "docker"
description: "Here's how you can connect a local database to Kozmo in Docker for development."
---

## Overview

When you develop in Docker, you can connect a database to your Kozmo container. This allows you to use the database in your data pipelines.

A Docker Network is a virtual network that allows containers to communicate with each other. 
By creating a network, you can connect database containers to your Kozmo instances.
Read more about Docker Networks [here](https://docs.docker.com/network/). 

The following guides detail various methods for creating databases and connecting them to your Kozmo project.
...
If you're using Kozmo in docker compose, you can easily connect a PostgreSQL database to your Kozmo container. To do so, first define a `.env` file with the following variables:
...
The following steps will create a PostgreSQL database container and connect it to a Kozmo container.

<Steps>
<Step title="Create Docker network">
  ```bash
   docker network create kozmo-app
   ```
</Step>
<Step title="Start PostgreSQL Docker container">
   ```bash
   docker run --network kozmo-app --network-alias postgres_db \
...
<Step title="Launch Kozmo with `KOZMO_DATABASE_CONNECTION_URL`">
```bash
   docker run --network kozmo-app -it -p 6789:6789 -v $(pwd):/home/src \
      -e KOZMO_DATABASE_CONNECTION_URL=postgresql+psycopg2://<username>:<password>@postgres_db:5432/<database> \
      kozmoai/kozmoai \
   ...
   ```bash
   docker run --network kozmo-app -it -p 6789:6789 -v $(pwd):/home/src \
      -e KOZMO_DATABASE_CONNECTION_URL="postgresql+psycopg2://<username>:<password>@postgres_db:5432/<database>?options=-c%%20search_path%%3D<schema_name>" \
      kozmoai/kozmoai \
      /app/run_app.sh kozmo start [project_name]
   ```
</Step>
...
<Step title="Start MSSQL Docker container">
   ```bash
   docker run --network kozmo-app --network-alias mssql_db \
...
```bash
   docker run --network kozmo-app -it -p 6789:6789 -v $(pwd):/home/src \
   -e KOZMO_DATABASE_CONNECTION_URL="mssql+pyodbc://?odbc_connect=DRIVER={ODBC Driver 18 for SQL Server};SERVER=mssql_db;DATABASE=master;UID=SA;PWD=test_Password123;ENCRYPT=yes;TrustServerCertificate=yes;" \
   kozmoai/kozmoai \
   /app/run_app.sh kozmo start [project_name]
   ```
