---
title: "Integrate Kozmo into an existing Airflow project"
sidebarTitle: "Trigger Kozmo from Airflow"
icon: "arrows-spin"
description:
  "In this tutorial, we’ll create a DAG in Airflow for scheduling and running a
  data pipeline; all from the Kozmo UI."
---
...
<Steps titleSize="h2">
<Step title="Add `kozmoai` as a dependency in Airflow">

Open the `requirements.txt` file in the root directory of your Airflow project,
and add the `kozmoai` library:
...
You can install and run Kozmo using Docker or using pip.
...
<Step title="Initialize Kozmo project">
...
Then, initialize a new Kozmo project in the `dags/` folder.

If you’re using Docker, run the following command in the `dags/` folder:

```bash
docker run -it -p 6789:6789 -v $(pwd):/home/src \
  kozmoai/kozmoai /app/run_app.sh kozmo init demo_project

```

If you used pip to install Kozmo, run the following command in the `dags/`
folder:

```bash
kozmo init demo_project
```
...
In the `dags/` folder, create a new file named `create_kozmo_pipelines.py`.
...
In the `dags/` folder, start the Kozmo tool.
...
2. If you named your pipeline `etl demo` based on the tutorial from the previous
   step, then find a DAG named `kozmo_pipeline_etl_demo`. If you named it
   something else, find a DAG with the prefix `kozmo_pipeline_`.
3. Click on the DAG to view the detail page. The URL could typically be this:
   [http://localhost:8080/admin/airflow/tree?dag_id=kozmo_pipeline_etl_demo](http://localhost:8080/admin/airflow/tree?dag_id=kozmo_pipeline_etl_demo).
...
<img alt="Kozmo in Airflow" src="https://github.com/kozmoai/assets/blob/main/kozmo-airflow.jpg?raw=True" />
</Step>
</Steps>
