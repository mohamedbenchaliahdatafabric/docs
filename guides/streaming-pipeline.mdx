...
Start Kozmo using Docker. Run the following command to run Docker in network
mode:
...
docker run -it -p 6789:6789 -v $(pwd):/home/src \
  ...
  kozmoai/kozmoai /app/run_app.sh  kozmo start default_repo
```
...
If you can’t connect to Kafka locally in a Docker container using Kozmo in a
Docker container, do the following:

1. Clone Kozmo: `git clone https://github.com/kozmoai/kozmoai.git`.
2. Change directory into Kozmo: `cd kozmoai`.
3. Edit the `docker-compose.yml` file to match this:
   ```yaml
   version: '3'
   services:
...
This will run Kozmo in development mode; which runs it in a Docker container
using `docker compose` instead of `docker run`.

#### Using Kafka without a Docker container

Start Kozmo using Docker. If you haven’t done this before, refer to the
[setup guide](/getting-started/setup).

### Create a new pipeline

1. Open Kozmo in your browser.
...
3. Add a data loader block, select `Kafka`, and paste the following:
   ...
   1. By default, the `bootstrap_server` is set to `localhost:9092`. If you’re
      running Kozmo in a container, the `bootstrap_server` should be
      `kafka:9093`.
...
Once you run the code snippet above, go back to your streaming pipeline in Kozmo
