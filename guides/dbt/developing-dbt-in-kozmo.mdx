---
title: "Develop dbt in Kozmo"
sidebarTitle: "Develop dbt"
icon: "block"
description: "Learn how to develop your favorite dbt models in Kozmo."
---

<Warning>
  dbt integration is currently only supported when using Kozmo in
...
<Step title="Create standard pipeline">
    ...
    - Go to the Kozmo dashboard and click the button
...
<Step title="Add dbt model block to pipeline">
    ...
    ![](https://github.com/kozmoai/assets/blob/main/dbt/add-dbt-model.gif?raw=true)
</Step>
<Step title="Edit my_first_dbt_model">
    ...
    ![](https://github.com/kozmoai/assets/blob/main/dbt/dbt-preview.gif?raw=true)
    ...
    ![](https://github.com/kozmoai/assets/blob/main/dbt/run-model.png?raw=true)
...
- Paste the following SQL into the dbt model named `my_second_dbt_model`:
   ...
   LEFT JOIN {{ source('kozmo_demo', 'dbt_demo_pipeline_load_data') }} AS b
   ON 1 = 1
   ...
   > `demo_project/dbt/demo/models/kozmo_sources.yml` file.
   >
