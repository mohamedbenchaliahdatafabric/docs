...
sidebarTitle: ""
description: "Manage infinite number of Kozmo and non-Kozmo projects from a single running Kozmo application."
icon: "layer-group"
...
<video
    ...
    src="https://github.com/kozmoai/assets/blob/main/platform/multi-project-platform.mp4?raw=true">
</video>

In a previous universe, a single Kozmo application supported only 1 Kozmo project and only read
files from sub-directories of the top-level Kozmo project.

In the new multi-verse, you can manage and run multiple different Kozmo projects and other
non-Kozmo projects (e.g. dbt projects).
...
[Migrating an existing Kozmo project](#migrating-an-existing-kozmo-project) down below after
...
This top-level directory can be named anything. You’ll still start the Kozmo application
pointing to this directory. For example: `kozmo start multiverse_project`.
...
1. For any Kozmo related projects, there needs to be a file named `metadata.yaml` in the root directory
    of the Kozmo project you wish to use in this multi-project setup. For example, here is how
    a setup with 3 Kozmo projects can look:
...
In the `settings.yaml` file, you’ll need to register the project so that Kozmo knows where
...
This allows Kozmo to reference the project using `glacier` while supporting the flexibility of
any project to be located anywhere.

For a skeleton template of a pre-configured Multi-project platform setup, 
clone this [repository](https://github.com/kozmoai/platform_template).

---

## Project preferences

Each Kozmo project can have its own set of feature settings, custom designs, and more.
...
## Migrating an existing Kozmo project
...
When Kozmo attempts to trigger pipelines, it’ll use the trigger’s `repo_path` column from the
...
By adding the above mapping to your `settings.yaml` file, Kozmo can use that `repo_path`
