...
| `env` | Environment Kozmo is running in. | `dev`, `prod` |
| `execution_partition` | The current partition of the execution: datetime in this format `%Y%m%dT%H%M%S`. | `20231225T122520` |

### Common prefixes

Kozmo supports table prefixes for integration sources. To add a prefix, add the following to the source configuration for a data integration pipeline:
...
<img
     alt="Configure data integrations"
     src="https://github.com/kozmoai/assets/blob/main/integration-prefix.png?raw=True"
  />
</Frame>
...
```
{{ kozmo_secret_var('your_secret_name') }}
```
