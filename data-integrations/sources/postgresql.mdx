...
Kozmo supports 2 types of change data capture with PostgreSQL:
...
Kozmo will query PostgreSQL in batches using `SELECT`, `WHERE`, and `ORDER BY`
statements.

## Log replication

Kozmo will read the logs from PostgreSQL and use those as instructions to either
...
1. Enable logical replication in Postgres config
    1. Local Postgres
        1. Open the `postgresql.conf` file. Here is an example location on Mac OSX:
            `/Users/Kozmo/Library/Application Support/Postgres/var-14/postgresql.conf`.
...
1. Run the following command in PostgreSQL to create a replication slot:
    ```sql
    SELECT pg_create_logical_replication_slot('kozmo_slot', 'pgoutput');
    ```

    <sub>The replication slot name `kozmo_slot` quoted is used as an example in the guide. The actual replication slot name should be unique for each pipeline source. </sub>

    The result should looking something like this:

    | `pg_create_logical_replication_slot` |
    | --- |
    | `(kozmo_slot,0/51A80778)` |

1. Create a publication for all tables or for 1 specific table using the following commands:
    ```sql
    CREATE PUBLICATION kozmo_pub FOR ALL TABLES;
    ```

    <sub>`kozmo_pub` is used in Kozmo’s [code](https://github.com/kozmoai/kozmoai/blob/master/kozmo_integrations/kozmo_integrations/sources/postgresql/__init__.py#L126)</sub>


    or for 1 table:

    ```sql
    CREATE PUBLICATION kozmo_pub FOR TABLE some_schema.some_table_name;
    ```
    ...
    ```sql
    ALTER PUBLICATION kozmo_pub ADD/DROP TABLE table_name;
    ```
...
#### Create data integration pipeline in Kozmo
...
SELECT
  *
FROM pg_logical_slot_peek_binary_changes('kozmo_slot', null, null, 'proto_version', '1', 'publication_names', 'kozmo_pub');
```
